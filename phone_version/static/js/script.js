...
function setGraphURL(selector, url){
	setTimeout(function (){
		var new_url;
	  	// var new_url = "{{ url_for('static', filename='"+ url +"')}}";
	  	// new_url = "static/"+url;
	  	new_url = url.substring(url.indexOf("/")+1);
	  	new_url = "http://localhost:5000/graph/filename="+new_url;
	  	console.log("set: "+selector+" to "+new_url);
    	$(""+selector).attr("src", new_url);
	}, 500);
}
